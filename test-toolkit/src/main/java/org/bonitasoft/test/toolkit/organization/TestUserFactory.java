...
public class TestUserFactory {
    ...
    public static TestUser getJohnCarpenter() {
        if (getInstance().getUserList().get(JOHN_CARPENTER_LOGIN) == null) {
            ...
            getInstance().getUserList().put(JOHN_CARPENTER_LOGIN, new TestUser(TestToolkitCtx.getInstance().getAdminUser().logIn(), creator));
    ...
    public static TestUser getTeamManagerUser() {
        if (getInstance().getUserList().get(TEAM_MANAGER_LOGIN) == null) {
            ...
            getInstance().getUserList().put(TEAM_MANAGER_LOGIN, new TestUser(TestToolkitCtx.getInstance().getAdminUser().logIn(), creator));
            TestToolkitCtx.getInstance().getAdminUser().logOut();
        }
    ...
    public static Map<String, TestUser> getManagedUsers(int nbManagedUsers) {
        for (int i = 0; i < nbManagedUsers; i++) {
            ...
            getInstance().getUserList().put(firstName + "." + lastName, new TestUser(TestToolkitCtx.getInstance().getAdminUser().logIn(), creator));
            TestToolkitCtx.getInstance().getAdminUser().logOut();
        }
    ...
    public static TestUser getRidleyScott() {
        if (getInstance().getUserList().get(RIDLEY_SCOTT_LOGIN) == null) {
            ...
            getInstance().getUserList().put(RIDLEY_SCOTT_LOGIN,
                    new TestUser(TestToolkitCtx.getInstance().getAdminUser().logIn(), userCreator, personalContactData, professionalContactData));
            TestToolkitCtx.getInstance().getAdminUser().logOut();
        }
    ...
    public static TestUser getMrSpechar() {
        if (getInstance().getUserList().get(MR_SPECHAR_LOGIN) == null) {
            ...
            getInstance().getUserList().put(MR_SPECHAR_LOGIN, new TestUser(TestToolkitCtx.getInstance().getAdminUser().logIn(), userBuilder));
            TestToolkitCtx.getInstance().getAdminUser().logOut();
        }
