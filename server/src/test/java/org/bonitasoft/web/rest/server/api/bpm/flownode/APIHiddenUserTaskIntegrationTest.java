...
public class APIHiddenUserTaskIntegrationTest extends AbstractConsoleTest {
     ...
     */
    @Test
    public void testDeleteHiddenUserTask() {
        testHumanTask.hide();

        final APIID apiid = APIID.makeAPIID(TestUserFactory.getJohnCarpenter().getId(), testHumanTask.getId());
        final List<APIID> apiids = new ArrayList<APIID>();
        apiids.add(apiid);
        apiHiddenUserTask.delete(apiids);

        Assert.assertTrue("Can't delete the HiddenUserTask with id=" + testHumanTask.getId() + " to the APIHiddenUserTAsk", testHumanTask.isPending());
        Assert.assertFalse("Can't delete the HiddenUserTask with id=" + testHumanTask.getId() + " to the APIHiddenUserTAsk", testHumanTask.isHidden());
    }
     ...
     */
    @Test
    public void testUserHideAdminAssign() {
        ...
        hiddenUserTaskItem.setTaskId(testHumanTask.getId());
        hiddenUserTaskItem = apiHiddenUserTask.add(hiddenUserTaskItem);

        Assert.assertFalse("Task still appears in pending tasks", testHumanTask.isPending());
        Assert.assertTrue("Task has disapeared", testHumanTask.isHidden());

        // change assignation
        testHumanTask.assignTo(TestUserFactory.getJohnCarpenter());
        Assert.assertFalse("Task is still hidden", testHumanTask.isHidden());
        Assert.assertFalse("Task should not be pending", testHumanTask.isPending());
        Assert.assertTrue("Task should be assigned",
                TestUserFactory.getJohnCarpenter().getAssignedTasks().contains(testHumanTask.getHumanTaskInstance()));
    }
     ...
     */
    @Test
    public void testGetHiddenUserTaskItem() {
        testHumanTask.hide();
        ...
        final HiddenUserTaskItem hidden = apiHiddenUserTask.runGet(apiid, deploys, counters);

        assertEquals(
                "Can't get the HiddenUserTask",
                testHumanTask.getId(), (long) hidden.getId().getPartAsAPIID(HiddenUserTaskItem.ATTRIBUTE_TASK_ID).toLong());
    }
     ...
     */
    @Test
    public void testAddHiddenUserTaskItem() {
        ...
        Assert.assertFalse("Can't hide the UserTask with id=" + testHumanTask.getId() + " to the APIHiddenUserTAsk", testHumanTask.isPending());
        Assert.assertTrue("Can't hide the UserTask with id=" + testHumanTask.getId() + " to the APIHiddenUserTAsk", testHumanTask.isHidden());
    }
     ...
     * @
     */
    private void createAPIHiddenUserTask() {
        apiHiddenUserTask = new APIHiddenUserTask();
        apiHiddenUserTask.setCaller(getAPICaller(TestUserFactory.getJohnCarpenter().getSession(),
