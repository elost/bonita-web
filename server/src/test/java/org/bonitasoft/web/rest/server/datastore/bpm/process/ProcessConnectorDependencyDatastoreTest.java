...
public class ProcessConnectorDependencyDatastoreTest extends APITestWithMock {
    ...
    public void initializeMocks() {
        ...
        datastore = spy(new ProcessConnectorDependencyDatastore(null));
    ...
    public void searchReturnAnEmptyResultIfNoConnectorImplementationIsFound() throws Exception {
        when(processAPI.getConnectorImplementation(anyLong(), anyString(), anyString())).thenReturn(null);
        final Map<String, String> filters = buildFilters(1L, "aConnectorName", "1");

        final ItemSearchResult<ProcessConnectorDependencyItem> searchResult = datastore.search(0, 10, null, null, filters);
    ...
    public void searchThrowExceptionIfProcessIdIsUnknown() throws Exception {
        when(processAPI.getConnectorImplementation(anyLong(), anyString(), anyString())).thenThrow(
                new ConnectorNotFoundException(new NullPointerException()));
        final Map<String, String> filters = buildFilters(1L, "aConnectorName", "1");

        datastore.search(0, 10, null, null, filters);
    }
    ...
    public void searchCanBePaginated() throws Exception {
        ...
        when(processAPI.getConnectorImplementation(1L, "connectorId", "connectorVersion")).thenReturn(connectorWith3Dependencies);
        final Map<String, String> filters = buildFilters(1L, "connectorId", "connectorVersion");

        final ItemSearchResult<ProcessConnectorDependencyItem> searchResult = datastore.search(0, 2, null, null, filters);
