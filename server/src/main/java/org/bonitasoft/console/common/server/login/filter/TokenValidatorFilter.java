...
public class TokenValidatorFilter extends AbstractAuthorizationFilter {
    ...
    @Override
    boolean checkValidCondition(final HttpServletRequest httpRequest, final HttpServletResponse httpResponse) {
        if (PropertiesFactory.getSecurityProperties().isCSRFProtectionEnabled()) {
            final String headerFromRequest = httpRequest.getHeader("X-Bonita-API-Token");
            final String apiToken = (String) httpRequest.getSession().getAttribute("api_token");
