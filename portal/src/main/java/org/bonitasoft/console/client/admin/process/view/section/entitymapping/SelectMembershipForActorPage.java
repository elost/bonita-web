...
public class SelectMembershipForActorPage extends SelectMembershipAndDoPageOnItem<ActorItem> {
    ...
    public SelectMembershipForActorPage() {
        super(ActorDefinition.get());
    }

    public SelectMembershipForActorPage(final String itemId) {
        super(itemId, ActorDefinition.get());
    }
                ...
                new APICaller(ActorMemberDefinition.get())
                        .add(item, new APICallback() {
                            ...
                            public void onSuccess(final int httpStatusCode, final String response, final Map<String, String> headers) {
                                ViewController.showPopup(ListProcessActorMembershipPage.TOKEN, MapUtil.asMap(
                                        new Arg("id", SelectMembershipForActorPage.this.getParameter("id"))));
                            }
    ...
    protected String defineSubmitButtonTooltip(final ActorItem item) {
        return _("Add this membership to %actor_name%", new Arg("actor_name", item.getAttributeValue(ActorItem.ATTRIBUTE_DISPLAY_NAME)));
    }

    @Override
    protected void defineTitle(final ActorItem item) {
        setTitle(_("Add a membership to %actor_name%", new Arg("actor_name", item.getAttributeValue(ActorItem.ATTRIBUTE_DISPLAY_NAME))));
